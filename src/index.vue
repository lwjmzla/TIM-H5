<template>
  <div class="container">
    ...
    <div class="loading" v-else v-loading="showLoading"  element-loading-text="正在拼命初始化..." element-loading-background="rgba(0, 0, 0, 0.8)">
...
export default {
  ...
  watch: {
    isLogin(next) {
      if (next) {
        //MTA.clickStat('link_two', { show: 'true' }) //!统计数据用的
      }
    }
  },
  ...
  methods: {
    initListener() {
      ...
      this.tim.on(this.TIM.EVENT.SDK_READY, this.onReadyStateUpdate, this) // !第三个参数 是context，这种形式下可以不需要，但function(event){}这样就要，es6()=>{}这种可能不要
      // SDK NOT READT
      this.tim.on(this.TIM.EVENT.SDK_NOT_READY, this.onReadyStateUpdate, this)
      // !被踢出 多端登录会呗踢出，如需支持多实例登录，请到 即时通信 IM 控制台，找到相应 SDKAppID，
      // !【应用配置】 > 【功能配置】> 【Web端实例同时在线】配置实例个数。配置将在50分钟内生效。
    ...
    onReadyStateUpdate(event) { // !{name: "sdkStateReady", data: undefined}
      const isSDKReady = event.name === this.TIM.EVENT.SDK_READY ? true : false // !因为SDK_READY和SDK_NOT_READY 都通过这函数进行处理
      this.$store.commit('toggleIsSDKReady', isSDKReady)

      if (isSDKReady) {
        this.tim.getMyProfile() // !这是获取我自己的资料，还有获取多个用户tim.getUserProfile({ userIDList: ['user1', 'user2']});
          .then(({ data }) => {
