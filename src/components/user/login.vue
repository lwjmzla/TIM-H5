...
export default {
  ...
  methods: {
    ...
    login() {
      this.loading = true
      this.tim.login({
          userID: this.form.userID, // !UID00024003
          userSig: window.genTestUserSig(this.form.userID).userSig // !
        })
        .then(() => {
          this.loading = false
          this.$store.commit('toggleIsLogin', true)
          this.$store.commit('startComputeCurrent') // !开启定时器state.intervalID，不断计算当前时间state.current
          this.$store.commit({
            type: 'GET_USER_INFO', // !给state.userID/userSig/sdkAppID赋值
            userID: this.form.userID, // !UID00024003
            userSig: window.genTestUserSig(this.form.userID).userSig,
